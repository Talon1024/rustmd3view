use glow::{
    Context as GLContext, HasContext, INVALID_ENUM,
    INVALID_FRAMEBUFFER_OPERATION, INVALID_OPERATION, INVALID_VALUE, NO_ERROR,
    OUT_OF_MEMORY,
};
use std::error::Error;
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            GLError::InvalidFramebufferOperation => {
                write!(f, "InvalidFramebufferOperation")
            }
    ...
    pub fn get(glc: &GLContext) -> Result<(), GLError> {
        match unsafe { glc.get_error() } {
            ...
            INVALID_FRAMEBUFFER_OPERATION => {
                Err(GLError::InvalidFramebufferOperation)
            }
